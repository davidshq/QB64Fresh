...
const char* qb_string_data(qb_string* s) {
    return s ? s->data : "";
}
...
#define QB_CMEM_SIZE 1114099
#define QB_DBLOCK_OFFSET 1280
static uint8_t qb_cmem[QB_CMEM_SIZE];
...
void qb_def_seg(int32_t segment) {
    ...
    if (segment == -1) {
        qb_defseg = &qb_cmem[QB_DBLOCK_OFFSET];
...
int qb_peek(int32_t offset) {
    if (offset < -65536 || offset > 65535) {
        return 0;
    }
    return qb_defseg[(uint16_t)offset];
}

void qb_poke(int32_t offset, uint8_t value) {
    if (offset < -65536 || offset > 65535) {
        return;
    }
    qb_defseg[(uint16_t)offset] = value;
}
...
static int32_t _qb_view_print_top = 1;
static int32_t _qb_view_print_bottom = 25;
...
void qb_gfx_get_step(int32_t x1, int32_t y1, int32_t w, int32_t h, void* arr) {
    /* STEP variant: w,h are relative offsets */
    qb_gfx_get(x1, y1, x1 + w, y1 + h, arr);
}

void qb_gfx_put(int32_t x, int32_t y, void* arr, int action, int clip, int32_t trans_color) {
    (void)x; (void)y; (void)arr; (void)action; (void)clip; (void)trans_color;
    _qb_gfx_warn();
    /* Stub: PUT graphics from array not yet implemented */
}

void qb_gfx_put_step(int32_t x, int32_t y, void* arr, int action, int clip, int32_t trans_color) {
...
int64_t qb_printwidth(qb_string* text) {
    ...
    return (int64_t)text->len * qb_fontwidth();
}
...
int qb_lpos(int64_t n) {
    (void)n;  // Printer number (ignored)
    return 1; // Always return column 1 (stub)
}

int32_t qb_varptr(void* ptr) {
    return (int32_t)(intptr_t)ptr;
}

qb_string* qb_varptr_str(void* ptr) {
    qb_string* result = malloc(sizeof(qb_string));
    result->len = sizeof(void*);
    result->capacity = result->len + 1;
    result->data = malloc(result->capacity);
    memcpy(result->data, &ptr, sizeof(void*));
    result->data[result->len] = '\0';
    return result;
}

int32_t qb_varseg(void* ptr) {
    (void)ptr;
    return 0; // Flat memory model, no segments
}

int32_t qb_sadd(qb_string* s) {
    if (!s || !s->data) return 0;
    return (int32_t)(intptr_t)(s->data);
}

int qb_fileattr(int filenum, int attribute) {
    (void)filenum; (void)attribute;
    // Stub: would need to track file modes in file table
    return 0;
}

float qb_cvsmbf(qb_string* s) {
    if (!s || s->len < 4) return 0.0f;
    unsigned char* mbf = (unsigned char*)s->data;
    // MBF format: mantissa(3 bytes) + exponent(1 byte)
    // IEEE format: sign(1) + exponent(8) + mantissa(23)
    if (mbf[3] == 0) return 0.0f; // Zero value
    uint32_t ieee;
    int sign = mbf[2] & 0x80;
...
double qb_cvdmbf(qb_string* s) {
    if (!s || s->len < 8) return 0.0;
    unsigned char* mbf = (unsigned char*)s->data;
    // MBF double: mantissa(7 bytes) + exponent(1 byte)
    if (mbf[7] == 0) return 0.0; // Zero value
    uint64_t ieee;
    int sign = mbf[6] & 0x80;
...
qb_string* qb_mksmbf(float n) {
    qb_string* result = malloc(sizeof(qb_string));
    result->len = 4;
    result->capacity = 5;
    result->data = malloc(5);
    if (n == 0.0f) {
        memset(result->data, 0, 4);
    } else {
        uint32_t ieee;
        memcpy(&ieee, &n, 4);
        int sign = (ieee >> 31) & 1;
        ...
        result->data[0] = mantissa & 0xFF;
        result->data[1] = (mantissa >> 8) & 0xFF;
        result->data[2] = ((mantissa >> 16) & 0x7F) | (sign << 7);
        result->data[3] = exp & 0xFF;
    }
    result->data[4] = '\0';
    return result;
}

qb_string* qb_mkdmbf(double n) {
    qb_string* result = malloc(sizeof(qb_string));
    result->len = 8;
    result->capacity = 9;
    result->data = malloc(9);
    if (n == 0.0) {
        ...
        result->data[0] = mantissa & 0xFF;
        result->data[1] = (mantissa >> 8) & 0xFF;
        result->data[2] = (mantissa >> 16) & 0xFF;
        result->data[3] = (mantissa >> 24) & 0xFF;
        result->data[4] = (mantissa >> 32) & 0xFF;
        result->data[5] = (mantissa >> 40) & 0xFF;
        result->data[6] = ((mantissa >> 48) & 0x7F) | (sign << 7);
        result->data[7] = exp & 0xFF;
    }
    result->data[8] = '\0';
    return result;
}
...
void qb_bload(const char* filename, void* address) {
    ...
    // Read BSAVE header (7 bytes)
    unsigned char header[7];
    if (fread(header, 1, 7, f) != 7) {
        fclose(f);
        return;
    }
    
    // Check magic byte (0xFD for BSAVE files)
    if (header[0] != 0xFD) {
        // Not a BSAVE file, load as raw binary
        fseek(f, 0, SEEK_END);
        long size = ftell(f);
        fseek(f, 0, SEEK_SET);
        if (address) {
            fread(address, 1, (size_t)size, f);
        }
        fclose(f);
        return;
    }
...
void qb_uevent_trigger(void) {
    // Trigger user-defined event stub
}

void qb_on_signal(int32_t signal_num, void* target) {
    (void)signal_num; (void)target;
}

void qb_signal_control(int32_t signal_num, int mode) {
    (void)signal_num; (void)mode;
}

/* QB4.5 Joystick Functions (stubs) */

int qb_stick(int64_t n) {
    (void)n;
    // Return center position (stub)
    // n=0,2: X coordinate, n=1,3: Y coordinate
    return 127; // Center of 0-255 range
}

int qb_strig(int64_t n) {
    (void)n;
    return 0; // Not pressed (stub)
}

/* QB4.5 Memory Functions (stubs) */

int32_t qb_fre(int64_t n) {
    (void)n;
...
void qb_free(void) {
    // String garbage collection is automatic
}

/* QB4.5 Port I/O Functions (stubs - sandboxed) */

int qb_inp(int64_t port) {
    (void)port;
...
void qb_out(int32_t port, int32_t value) {
    (void)port; (void)value;
    // Port I/O is not available on protected-mode systems
}

void qb_wait(int32_t port, int32_t and_mask, int32_t xor_mask) {
    (void)port; (void)and_mask; (void)xor_mask;
    // WAIT is not available on protected-mode systems
    // In original BASIC: loops until (INP(port) XOR xor_mask) AND and_mask <> 0
}

/* QB4.5 Light Pen Function (stub) */

int qb_pen(int64_t n) {
    (void)n;
    return 0; // Light pen not present
}

/* QB4.5 Serial I/O Functions (stubs) */

int qb_erdev(void) {
    return 0; // No device error
}

qb_string* qb_erdev_str(void) {
    return qb_string_new(""); // No device error
}

void qb_ioctl(int32_t file_num, qb_string* control_string) {
    (void)file_num; (void)control_string;
    // Device control string stub
}

qb_string* qb_ioctl_str(int64_t file_num) {
    (void)file_num;
    return qb_string_new(""); // Empty status string
}

/* QB4.5 System Interrupt Functions (stubs) */

void qb_interrupt(int32_t int_num, void* in_regs, void* out_regs) {
    (void)int_num; (void)in_regs; (void)out_regs;
    // System interrupts not available on protected-mode systems
    fprintf(stderr, "Warning: INTERRUPT statement is not supported on modern systems\n");
}

void qb_interruptx(int32_t int_num, void* in_regs, void* out_regs) {
    (void)int_num; (void)in_regs; (void)out_regs;
    fprintf(stderr, "Warning: INTERRUPTX statement is not supported on modern systems\n");
}
...
int main(int argc, char** argv) {
    ...
    qb_gfx_screen((int32_t)12LL, (int32_t)-1, (int32_t)-1, (int32_t)-1);
    const int32_t GRID_SIZE = 20LL;
    const int32_t GRID_WIDTH = 30LL;
    const int32_t GRID_HEIGHT = 22LL;
    const int32_t OFFSET_X = 40LL;
    const int32_t OFFSET_Y = 30LL;
    int16_t* snakeX = malloc(sizeof(int16_t) * (501));
    int16_t* snakeY = malloc(sizeof(int16_t) * (501));
    int16_t snakeLength = 0;
    int16_t direction = 0;
    int16_t foodX = 0;
    int16_t foodY = 0;
    ...
    int16_t gameOver = 0;
    int16_t speed = 0;
    qb_randomize((double)(qb_timer()));
    _gosub_stack[_gosub_sp++] = &&_qb_gosub_ret_0;
    goto InitGame;
    _qb_gosub_ret_0:;
    do {
        k = (float)(qb_keyhit());
        float _qb_select_2 = k;
        if ((_qb_select_2 == 18432LL)) {
            if ((direction != 1LL)) {
                direction = (int16_t)(3LL);
            }
        } else if ((_qb_select_2 == 20480LL)) {
            if ((direction != 3LL)) {
                direction = (int16_t)(1LL);
            }
        } else if ((_qb_select_2 == 19200LL)) {
            if ((direction != 0LL)) {
                direction = (int16_t)(2LL);
            }
        } else if ((_qb_select_2 == 19712LL)) {
            if ((direction != 2LL)) {
                direction = (int16_t)(0LL);
            }
        } else if ((_qb_select_2 == 27LL)) {
            gameOver = (int16_t)(1LL);
        }
        _gosub_stack[_gosub_sp++] = &&_qb_gosub_ret_3;
        goto UpdateSnake;
        _qb_gosub_ret_3:;
        _gosub_stack[_gosub_sp++] = &&_qb_gosub_ret_4;
        goto DrawGame;
        _qb_gosub_ret_4:;
        qb_limit((int)speed);
    } while (!(gameOver));
    _qb_do_end_1:;
    qb_gfx_color((int32_t)15LL, (int32_t)-1);
    qb_gfx_locate((int32_t)14LL, (int32_t)25LL);
    qb_print_string(qb_string_new("GAME OVER!"));
    qb_print_newline();
    qb_gfx_locate((int32_t)16LL, (int32_t)23LL);
    qb_print_string(qb_string_new("Final Score:"));
    qb_print_int(score);
    qb_print_newline();
    qb_gfx_locate((int32_t)18LL, (int32_t)20LL);
    qb_print_string(qb_string_new("Press any key to exit..."));
    qb_print_newline();
    qb_sleep_keypress();
    exit(0);
InitGame:
    snakeLength = (int16_t)(3LL);
    snakeX[0LL - 0] = (int16_t)((GRID_WIDTH / 2LL));
    snakeY[0LL - 0] = (int16_t)((GRID_HEIGHT / 2LL));
    snakeX[1LL - 0] = (int16_t)((snakeX[0LL - 0] - 1LL));
    snakeY[1LL - 0] = snakeY[0LL - 0];
    snakeX[2LL - 0] = (int16_t)((snakeX[0LL - 0] - 2LL));
    snakeY[2LL - 0] = snakeY[0LL - 0];
    direction = (int16_t)(0LL);
    score = (int16_t)(0LL);
    gameOver = (int16_t)(0LL);
    speed = (int16_t)(8LL);
    _gosub_stack[_gosub_sp++] = &&_qb_gosub_ret_5;
    goto PlaceFood;
    _qb_gosub_ret_5:;
...
PlaceFood:
    ...
    do {
        valid = (int16_t)(1LL);
        foodX = (int16_t)(floor((RND * GRID_WIDTH)));
        foodY = (int16_t)(floor((RND * GRID_HEIGHT)));
        int32_t _qb_for_end_val_8 = (snakeLength - 1LL);
        int32_t _qb_for_step_9 = 1;
        for (int32_t i = 0LL; (_qb_for_step_9 > 0) ? (i <= _qb_for_end_val_8) : (i >= _qb_for_end_val_8); i += _qb_for_step_9) {
            if (((foodX == snakeX[i - 0]) & (foodY == snakeY[i - 0]))) {
                valid = (int16_t)(0LL);
                goto _qb_for_end_7;
            }
        }
...
UpdateSnake:
    ...
    newX = snakeX[0LL - 0];
    newY = snakeY[0LL - 0];
    int16_t _qb_select_10 = direction;
    if ((_qb_select_10 == 0LL)) {
        newX = (int16_t)((newX + 1LL));
    } else if ((_qb_select_10 == 1LL)) {
        newY = (int16_t)((newY + 1LL));
    } else if ((_qb_select_10 == 2LL)) {
        newX = (int16_t)((newX - 1LL));
    } else if ((_qb_select_10 == 3LL)) {
        newY = (int16_t)((newY - 1LL));
    }
    if (((((newX < 0LL) | (newX >= GRID_WIDTH)) | (newY < 0LL)) | (newY >= GRID_HEIGHT))) {
        gameOver = (int16_t)(1LL);
        if (_gosub_sp > 0) goto *_gosub_stack[--_gosub_sp];
    }
    int32_t _qb_for_end_val_12 = (snakeLength - 1LL);
    int32_t _qb_for_step_13 = 1;
    for (int32_t i = 0LL; (_qb_for_step_13 > 0) ? (i <= _qb_for_end_val_12) : (i >= _qb_for_end_val_12); i += _qb_for_step_13) {
        if (((newX == snakeX[i - 0]) & (newY == snakeY[i - 0]))) {
            gameOver = (int16_t)(1LL);
            if (_gosub_sp > 0) goto *_gosub_stack[--_gosub_sp];
        }
    }
    ...
    if (((newX == foodX) & (newY == foodY))) {
        ate = (int16_t)(1LL);
        score = (int16_t)((score + 10LL));
        if ((speed < 20LL)) {
            speed = (int16_t)((speed + 1LL));
        }
        _gosub_stack[_gosub_sp++] = &&_qb_gosub_ret_14;
        goto PlaceFood;
        _qb_gosub_ret_14:;
    }
    if (ate) {
        snakeLength = (int16_t)((snakeLength + 1LL));
    }
    int32_t _qb_for_end_val_16 = 1LL;
    int32_t _qb_for_step_17 = (-1LL);
    for (int32_t i = (snakeLength - 1LL); (_qb_for_step_17 > 0) ? (i <= _qb_for_end_val_16) : (i >= _qb_for_end_val_16); i += _qb_for_step_17) {
        snakeX[i - 0] = snakeX[(i - 1LL) - 0];
        snakeY[i - 0] = snakeY[(i - 1LL) - 0];
    }
    _qb_for_end_15:;
    snakeX[0LL - 0] = newX;
    snakeY[0LL - 0] = newY;
...
DrawGame:
    qb_gfx_cls();
    qb_gfx_box_ex((int32_t)(OFFSET_X - 2LL), (int32_t)(OFFSET_Y - 2LL), (int32_t)((OFFSET_X + (GRID_WIDTH * GRID_SIZE)) + 1LL), (int32_t)((OFFSET_Y + (GRID_HEIGHT * GRID_SIZE)) + 1LL), 0, (uint32_t)7LL, 0);
    int16_t fx = 0;
    int16_t fy = 0;
    fx = (int16_t)((OFFSET_X + (foodX * GRID_SIZE)));
    fy = (int16_t)((OFFSET_Y + (foodY * GRID_SIZE)));
    qb_gfx_box_ex((int32_t)(fx + 2LL), (int32_t)(fy + 2LL), (int32_t)((fx + GRID_SIZE) - 3LL), (int32_t)((fy + GRID_SIZE) - 3LL), 0, (uint32_t)12LL, 1);
    int32_t _qb_for_end_val_19 = (snakeLength - 1LL);
    int32_t _qb_for_step_20 = 1;
    for (int32_t i = 0LL; (_qb_for_step_20 > 0) ? (i <= _qb_for_end_val_19) : (i >= _qb_for_end_val_19); i += _qb_for_step_20) {
        int16_t sx = 0;
        int16_t sy = 0;
        sx = (int16_t)((OFFSET_X + (snakeX[i - 0] * GRID_SIZE)));
        sy = (int16_t)((OFFSET_Y + (snakeY[i - 0] * GRID_SIZE)));
        if ((i == 0LL)) {
            qb_gfx_box_ex((int32_t)(sx + 1LL), (int32_t)(sy + 1LL), (int32_t)((sx + GRID_SIZE) - 2LL), (int32_t)((sy + GRID_SIZE) - 2LL), 0, (uint32_t)10LL, 1);
        } else {
            qb_gfx_box_ex((int32_t)(sx + 2LL), (int32_t)(sy + 2LL), (int32_t)((sx + GRID_SIZE) - 3LL), (int32_t)((sy + GRID_SIZE) - 3LL), 0, (uint32_t)2LL, 1);
        }
    }
    _qb_for_end_18:;
    qb_gfx_color((int32_t)15LL, (int32_t)-1);
    qb_gfx_locate((int32_t)1LL, (int32_t)1LL);
    qb_print_string(qb_string_new("Score:"));
    qb_print_int(score);
    qb_print_newline();
    qb_gfx_locate((int32_t)1LL, (int32_t)60LL);
    qb_print_string(qb_string_new("ESC to quit"));
    qb_print_newline();
    qb_gfx_locate((int32_t)30LL, (int32_t)25LL);
    qb_print_string(qb_string_new("Use Arrow Keys to move"));
    qb_print_newline();
    if (_gosub_sp > 0) goto *_gosub_stack[--_gosub_sp];
