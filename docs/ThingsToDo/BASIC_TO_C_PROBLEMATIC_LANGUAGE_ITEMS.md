...
**Last Updated:** 2026-01-31 (RESUME/ERL/ERR full implementation reflected)  
...
- [QB64pe/QB64Fresh_VS_QB64pe_DIFFERENCES.md](QB64pe/QB64Fresh_VS_QB64pe_DIFFERENCES.md) — QB64Fresh vs QB64pe (intentional and behavioral differences)
- [adrs/ADR-0008-c-interoperability.md](adrs/ADR-0008-c-interoperability.md) — C interop type mapping
- [TODO_CONSOLIDATED.md](TODO_CONSOLIDATED.md#option-b-complete-runtime-library) — Option B: runtime/error-system scope and implementation plan
- [ThingsToDo/PARTIAL_IMPLEMENTATIONS.md](ThingsToDo/PARTIAL_IMPLEMENTATIONS.md) — partial or stub implementations (e.g. ON ERROR _NEWHANDLER)
...
| **LINE INPUT # string lifetime** | `qb_file_line_input` assigned `*s = qb_string_new(buf)` which registered in temp pool; later cleanup freed it → dangling pointer. | Release old `*s` first; then assign `*s = qb_string_retain(qb_string_new(buf))` so variable owns reference and cleanup doesn't free it. |
| **INPUT # string lifetime** | Same issue as LINE INPUT #: temp pool cleanup freed the string variable. | Same fix: release old, retain new on assignment. |
...
| **REDIM PRESERVE parsing** | Lexer only recognized `_PRESERVE` (with underscore); `PRESERVE` (no underscore) was treated as variable name → parse error. | Parser accepts identifier "PRESERVE" (case-insensitive) as well as `TokenKind::Preserve` (_PRESERVE). |
| **Dynamic array vs scalar (empty dimensions)** | `DIM a() AS LONG` was treated as scalar (empty dimensions ⇒ scalar) → emitted `int32_t a = 0` instead of `int32_t* a = NULL` and size var. | Distinguish "no parentheses" (scalar) from "empty parentheses" (dynamic array) via `is_dynamic_array: bool` in AST/TypedIR; codegen declares pointer + size var for dynamic arrays. |
...
| **ON ERROR GOTO _NEWHANDLER label** | QB64 extension: `_NEWHANDLER` modifier pushes a new error handler scope. Parser must combine `_NEWHANDLER` with following label as one statement. | Parser treats `_NEWHANDLER` + label as one statement; codegen strips prefix and uses the label. Parser unit test, bootstrap test, and integration test verify. Documented in `src/codegen/c_backend/stmt/error_jump.rs` and [PARTIAL_IMPLEMENTATIONS](ThingsToDo/PARTIAL_IMPLEMENTATIONS.md). |
| **ERR/ERL in ON ERROR handler** | After `ERROR 5`, we emitted `qb_error(5)` but never jumped to handler. Execution continued, so handler never ran and ERR/ERL were never printed. | Codegen emits jump after ERROR: inline uses `if (_qb_error_handler) { goto *_qb_error_handler; }`; external uses `if (qb_error_pending()) { if (_qb_error_handler) { qb_commit_error(); goto *_qb_error_handler; } }`. Inline runtime sets `_qb_erl = 0` in `qb_error()` so ERL is defined when handler runs. |
| **RESUME (retry) and retry labels** | RESUME with no target must jump back to the statement that failed; runtime-originated errors (OPEN, PRINT #, SHELL, etc.) had no retry point. | Codegen emits a unique retry label before each failing call (file I/O, system), sets `_qb_error_line = &&retry_label` when jumping to handler; RESUME (no target) emits `goto *_qb_error_line` and clears it. ERROR statement also sets retry label so RESUME retries the ERROR. |
...
| **Resume behavior** | RESUME, RESUME NEXT, RESUME line — subtle order and state. | Fully implemented (retry labels for RESUME/RESUME label, `qb_clear_error` for RESUME NEXT, ERL/ERR preserved). ERL may be 0 when error originates in runtime (e.g. file I/O); edge cases may remain vs QB64pe. |
...
| **REDIM PRESERVE token** | Parser must accept both `_PRESERVE` (token) and `PRESERVE` (identifier) for REDIM statements. | Parser accepts identifier "PRESERVE" (case-insensitive) as well as `TokenKind::Preserve` (_PRESERVE). |
...
|----------|-------------------------------------|---------------------------|
| Strings | 10 items (types, double-wrap, temp pool, fixed-length, SELECT CASE, C API, LINE INPUT #, INPUT #) | 3 (BYREF to C, long concat, literal lifetime) |
...
| Arrays | 8 items (array vs call, rename, OPTION BASE, REDIM SHARED, scope, shadowing, REDIM PRESERVE parsing, dynamic array vs scalar) | 3 (LBOUND/UBOUND inline, huge arrays, OPTION BASE scope) |
| Control flow | 8 items (labels, forwards, EXIT/CONTINUE/RETURN, FOR/NEXT, ON ERROR _NEWHANDLER, ERR/ERL in handler, RESUME retry labels) | 3 (cross-proc GOTO, ON ERROR 0, Resume) |
| C interop | 3 items (STRING→C, return type, ALIAS) | 4 (BYREF STRING, structs, macros, platform) |
| Parser/Lexer | 8 items ($CONSOLE, SUB args, comparison, STATIC, IF-THEN-ELSE, DATA, empty dims, REDIM PRESERVE token) | 3 (reserved words, line numbers, colons) |
