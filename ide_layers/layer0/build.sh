#!/usr/bin/env bash
# Layer 0: build minimal window (C + runtime only).
# Run from QB64Fresh repo root: ide_layers/layer0/build.sh
set -euo pipefail
...
gcc -I "$QB64FRESH_ROOT/runtime/include" main.c \
  ...
  -lm -lpthread -ldl -o layer0
echo "Built: $SCRIPT_DIR/layer0"
